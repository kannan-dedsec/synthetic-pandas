...
def group_and_transform(df: pd.DataFrame, group_by_cols: list = [], transform_func: Callable[[pd.Series], Any] = None) -> pd.Series:
    """
...
def group_filter(df: pd.DataFrame, group_by_cols: list, filter_func: Callable[[pd.DataFrame], bool] = None) -> pd.DataFrame:
    """
...
def apply_custom_func(df: pd.DataFrame, group_by_cols: list, custom_func: Callable[[pd.DataFrame], Any], extra_params: dict = {}) -> pd.DataFrame:
    """
    ...
    extra_params (dict): Additional parameters for the custom function.
