...
def groupedAgg(df: pd.DataFrame, group_by: List[str], agg_funcs: Dict[str, Any]) -> pd.DataFrame:
    """
...
def custom_agg(df: pd.DataFrame, groupBy: List[str], **agg_kwargs: Dict[str, Any]) -> pd.DataFrame:
    """
    ...
    Returns:
        pd.DataFrame: A DataFrame containing the aggregated results.
    """
    return df.groupby(groupBy).agg(**agg_kwargs).reset_index()